...
This lecture relies on a small, focused toolkit organized by role. For data wrangling and visualization, we use the `tidyverse` to cover the essentials, while `timetk` streamlines time-aware feature engineering, visualization, and preprocessing.  
...
```{r, echo=FALSE}
email_tbl <- load_data("../../data/email", "email_prep")
...
```{r, echo=FALSE}
m4_hourly_tbl <- load_data("../../data/m4", "m4_prep_sample")
m4_hourly_tbl |> plot_time_series(ds, y, .facet_vars = unique_id, .facet_ncol = 2, .smooth = FALSE)
```
